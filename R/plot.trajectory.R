...
plot.trajectory = function(x, threeD = FALSE, col = "#2980b9", col_start = "#e67e22",
  ...
  if (x$system == "ellipsoidal"){
    if (threeD){
      if (threeD_col_grad){
        p = plot_ly(traj, x = ~lat*180/pi, y = ~lon*180/pi, z = ~alt, type = 'scatter3d', mode = 'lines',
                    line = list(width = threeD_line_width, color = 1:n,
      ...
      label = c(paste("Start - location; time: ", traj[1,1]),
                paste("End - location; time: ", traj[n,1]))
      
      leaflet(data = traj) %>% addTiles() %>%
        addMarkers(lng = ~lon*180/pi,
                   lat = ~lat*180/pi, data = x$trajectory[c(1,n),],
                   label = label) %>%
        addPolylines(lng = ~lon*180/pi,
                     lat = ~lat*180/pi,
                     data = x$trajectory)
    }
  }else{
    if (threeD){
      if (threeD_col_grad){
        p = plot_ly(traj, x = ~x_E, y = ~x_N, z = ~-x_D, type = 'scatter3d', mode = 'lines',
                    line = list(width = threeD_line_width, color = 1:n,
                                colorscale = list(c(0,threeD_grad_start), c(1,threeD_grad_end))))
      }else{
        p = plot_ly(traj, x = ~x_E, y = ~x_N, z = ~-x_D, type = 'scatter3d', mode = 'lines',
      ...
      plot(traj$x_E, traj$x_N, col = col, type = "l",
      ...
      if (!is.null(n_split) && add_altitude){
        ...
        for (i in 1:n_split){
          text(traj$x_E[index[i]] , traj$x_N[index[i]] + 0.015*diff(range(traj$x_N)),
               as.character(round(traj$time[index[i]]),1), cex = 0.8, col = col_start)
        }
      }
      ...
      if (add_altitude){
        ...
        plot(traj$time, -traj$x_D, col = col, type = "l",
             xlab= "Time (sec)",
             ylab = "Local - Up - Coord.")
